...
const results = { passed: [], failed: [], consoleErrors: [], debug: {} };
...
try {
  ...
  await check("direct source is verified before opening", async () => {
    ...
    results.debug.sourceFound = text;
  ...
  await check("source preparation shows real stages and waits for open", async () => {
    ...
    results.debug.sourceReady = text;
  ...
  const readerOpened = await check("prepared source opens physical reader", async () => {
  ...
  if (readerOpened) {
    ...
    await check("reader closes cleanly", async () => {
  ...
  await check("memory description returns candidate choices", async () => {
    ...
    const text = await page.locator(".discovery-results-card").innerText();
    results.debug.discovery = { count, text };
  ...
  results.failed.push({ name: "unhandled test harness error", error: error instanceof Error ? error.message : String(error) });
} finally {
