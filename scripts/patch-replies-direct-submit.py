...
new = '''  const [draft, setDraft] = useState(""); const listRef = useRef<HTMLDivElement>(null); const submitting = useRef(false); useEffect(() => { const list = listRef.current; if (!list) return; window.requestAnimationFrame(() => { list.scrollTop = list.scrollHeight; }); }, [replies.length]); function sendDraft() { const clean = draft.trim(); if (!clean || submitting.current) return; submitting.current = true; onSend(clean); setDraft(""); window.requestAnimationFrame(() => { submitting.current = false; }); } function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); sendDraft(); }\n'''
if text.count(old) != 1:
...
new_button = '<button type="button" onClick={sendDraft} disabled={!draft.trim()}>Send</button>'
...
print("Replies direct tap/Enter submit ownership applied.")
