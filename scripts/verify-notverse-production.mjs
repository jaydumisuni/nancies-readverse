...
const targets = [
  { id: "custom", url: "https://nancyreadverse.link.online", required: false },
  { id: "worker", url: "https://nancies-readverse.pharrtechnolgiescoltd.workers.dev", required: true },
];
...
async function verifyTarget(browser, target) {
  const result = { id: target.id, url: target.url, required: target.required, ok: false, desktop: {}, mobile: {}, errors: [] };
  try {
    for (const mode of [
...
try {
  ...
  const failedRequired = report.targets.filter((target) => target.required && !target.ok);
  assert(failedRequired.length === 0, failedRequired.map((target) => `${target.id}: ${target.error || target.errors.join(" | ")}`).join("; "));
  report.ok = true;
