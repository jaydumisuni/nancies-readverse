...
const [app, setup, views, notes, css, worker, platform, social, schema, socialConfig, manifest, index, packageJson, wrangler] = await Promise.all([
  ...
  read("src/notverse/NotesSocialExperience.tsx"),
...
const checks = [
  ...
  [notes.includes("RepliesDrawer"), "Comments surface is missing"],
  ...
  [!views.includes("Provider connections pending") && !views.includes("No fake score shown"), "obsolete rating placeholder remains"],
  ...
  [social.includes("No public rating data is available for this edition yet"), "public rating availability guard is missing"],
