...
async function clickNavigation(page, label, mobile) {
  ...
  const button = page.locator(`${root} button`).filter({ hasText: new RegExp(`^\s*${label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\s*$`, "i") }).first();
  await button.click();
...
async function assertFocusedInput(page, selector, label, mobile) {
  ...
  assert(state.box.left >= -1 && state.box.right <= state.viewport.width + 1, `${label}: focused input leaves the viewport horizontally ${JSON.stringify(state)}`);
  assert(state.box.top >= -1 && state.box.bottom <= state.viewport.height + 1, `${label}: focused input leaves the viewport vertically ${JSON.stringify(state)}`);
  if (mobile) {
...
try {
  ...
  for (const viewport of viewports) {
    ...
    if (viewport.mobile) {
      ...
      assert((keyboard.panelBottom || 0) <= keyboard.viewportHeight + 1, `${viewport.name}/Keyboard: chat panel exceeds resized viewport ${JSON.stringify(keyboard)}`);
      assert((keyboard.composerBottom || 0) <= keyboard.viewportHeight + 1, `${viewport.name}/Keyboard: composer hides behind software keyboard area ${JSON.stringify(keyboard)}`);
