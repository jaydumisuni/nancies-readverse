...
function RepliesDrawer({ note, onClose }: { note: NoTVerseNote; onClose: () => void }) {
  return <div className="note-modal-backdrop replies-backdrop"><section className="replies-drawer"><header><div><strong>Replies</strong><small>{note.replies} responses to this Note</small></div><button type="button" onClick={onClose}>×</button></header><div className="replies-list"><article><b>Mugiwara_05</b><p>I totally agree. That scene gave me chills. The promise hits differently every time.</p><small>2h ago · Spoiler boundary respected</small></article><article><b>PageTurner</b><p>The quiet panel before it is what made the moment work for me.</p><small>1h ago</small></article></div><form onSubmit={(event) => event.preventDefault()}><input placeholder="Reply without spoiling what comes next…" /><button type="submit">Send</button></form></section></div>;
}
