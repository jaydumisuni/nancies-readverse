...
function InboxView({ displayName }: { displayName: string }) {
  ...
  return <section className="notverse-view inbox-view"><header><span className="notverse-eyebrow">Private conversations</span><h1>Inbox</h1><p>Messages, Note shares, book shares and Notebook invitations.</p></header><div className="inbox-layout"><aside>{threads.map((thread) => <button type="button" className={selected === thread.id ? "active" : ""} key={thread.id} onClick={() => { setSelected(thread.id); setThreads((current) => current.map((item) => item.id === thread.id ? { ...item, unread: 0 } : item)); }}><span>{thread.name.slice(0, 1)}</span><div><strong>{thread.name}</strong><small>{thread.preview}</small></div><time>{thread.time}</time>{thread.unread > 0 && <b>{thread.unread}</b>}</button>)}</aside><main>{active ? <><header><span>{active.name.slice(0, 1)}</span><div><strong>{active.name}</strong><small>Private conversation with {displayName}</small></div><button type="button">•••</button></header><div className="message-thread"><p className="received">That panel is exactly what I meant.</p><p className="sent">The page turn made it land even harder.</p><p className="received">I shared the Note. Spoiler boundary is set to Chapter 34.</p></div><form onSubmit={(event) => event.preventDefault()}><input placeholder="Write a private message…" /><button>Send</button></form></> : <p>Select a conversation.</p>}</main></div></section>;
}
