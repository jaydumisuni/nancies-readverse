...
export default function NotesSocialExperience({ displayName, avatar, libraryTitles, noteFont, spoilerProgress = {} }: Props) {
  ...
  return <section className={`notes-experience notes-social-experience note-font-${noteFont}`} onPointerDown={pointerDown} onPointerUp={pointerUp}>
    ...
    {repliesOpen && note && <RepliesDrawer note={note} replies={repliesByNote[note.id] || []} onClose={() => setRepliesOpen(false)} onSend={submitReply} />}
    {activityOpen && <ActivityPanel activities={activities} notes={notes} onClose={() => { setActivityOpen(false); window.history.replaceState(null, "", "#my-notes"); }} onOpenNote={openActivityNote} onMarkRead={() => setActivities((current) => current.map((item) => ({ ...item, unread: false })))} />}
...
function NoteComposer({ displayName, avatar, libraryTitles, noteFont, onClose, onPublish }: { displayName: string; avatar?: string; libraryTitles: string[]; noteFont: string; onClose: () => void; onPublish: (note: NoTVerseNote) => void }) {
  ...
  function submit() { if (!text.trim()) return; onPublish({ id: `note-${Date.now()}`, author: displayName || "Reader", avatar, notebook: "My Notebook", createdAt: "Just now", text: text.trim(), type, visibility: visible, book: book || undefined, chapter: chapter || undefined, spoilerBoundary: spoiler, tags: [], image, reactions: 0, replies: 0, saved: true, mine: true }); }
...
function RepliesDrawer({ note, replies, onClose, onSend }: { note: NoTVerseNote; replies: NoteReply[]; onClose: () => void; onSend: (text: string) => void }) {
  const [draft, setDraft] = useState(""); function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const clean = draft.trim(); if (!clean) return; onSend(clean); setDraft(""); }
  return <div className="note-modal-backdrop replies-backdrop"><section className="replies-drawer"><header><div><strong>Replies</strong><small>{replies.length} saved here{note.replies > replies.length ? ` · ${note.replies} total` : ""}</small></div><button type="button" onClick={onClose} aria-label="Close replies">×</button></header><div className="replies-list">{replies.length === 0 ? <div className="replies-empty"><strong>No replies yet.</strong><p>Start the conversation without spoiling what comes next.</p></div> : replies.map((reply) => <article key={reply.id} data-reply-id={reply.id}><span className="reply-avatar">{reply.avatar ? <img src={reply.avatar} alt="" /> : reply.author.slice(0, 1)}</span><div><b>{reply.author}{reply.mine && <em>You</em>}</b><p>{reply.text}</p><small>{reply.createdAt}</small></div></article>)}</div><form onSubmit={submit}><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Reply without spoiling what comes next…" aria-label="Write a reply" /><button type="submit" disabled={!draft.trim()}>Send</button></form></section></div>;
}
...
function ActivityPanel({ activities, notes, onClose, onOpenNote, onMarkRead }: { activities: NoteActivity[]; notes: NoTVerseNote[]; onClose: () => void; onOpenNote: (noteId: string) => void; onMarkRead: () => void }) {
  return <div className="note-modal-backdrop activity-backdrop" onClick={onClose}><section className="note-activity-panel" onClick={(event) => event.stopPropagation()}><header><div><strong>Activity</strong><small>Replies, reactions, shares and your Note history.</small></div><button type="button" onClick={onClose} aria-label="Close activity">×</button></header><div className="activity-toolbar"><span>{activities.filter((item) => item.unread).length} unread</span><button type="button" onClick={onMarkRead}>Mark all read</button></div><div className="activity-list">{activities.length === 0 ? <div className="activity-empty"><strong>No Note activity yet.</strong><p>When you publish, reply, save or share, your Note history will appear here.</p></div> : activities.map((item) => { const target = notes.find((entry) => entry.id === item.noteId); return <button type="button" key={item.id} className={item.unread ? "unread" : ""} onClick={() => onOpenNote(item.noteId)}><span>{item.kind === "reply" ? "▢" : item.kind === "reaction" ? "♥" : item.kind === "share" ? "↗" : "•"}</span><div><strong>{item.title}</strong><p>{item.detail}</p><small>{target ? `Open “${target.text.slice(0, 55)}${target.text.length > 55 ? "…" : ""}”` : "Note no longer available"} · {item.createdAt}</small></div></button>; })}</div></section></div>;
}
