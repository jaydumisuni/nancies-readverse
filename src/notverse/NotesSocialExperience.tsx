...
function RepliesDrawer({ note, replies, onClose, onSend }: { note: NoTVerseNote; replies: NoteReply[]; onClose: () => void; onSend: (text: string) => void }) {
  ...
  return <div className="note-modal-backdrop replies-backdrop"><section className="replies-drawer"><header><div><strong>Replies</strong><small>{replies.length} saved here{note.replies > replies.length ? ` · ${note.replies} total` : ""}</small></div><button type="button" onClick={onClose} aria-label="Close replies">×</button></header><div className="replies-list" ref={listRef}>{replies.length === 0 ? <div className="replies-empty"><strong>No replies yet.</strong><p>Start the conversation without spoiling what comes next.</p></div> : replies.map((reply) => <article key={reply.id} data-reply-id={reply.id}><span className="reply-avatar">{reply.avatar ? <img src={reply.avatar} alt="" /> : reply.author.slice(0, 1)}</span><div><b>{reply.author}{reply.mine && <em>You</em>}</b><p>{reply.text}</p><small>{reply.createdAt}</small></div></article>)}</div><form onSubmit={submit}><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Reply without spoiling what comes next…" aria-label="Write a reply" /><button type="button" onPointerDown={(event) => { event.preventDefault(); sendDraft(); }} onClick={sendDraft} disabled={!draft.trim()}>Send</button></form></section></div>;
}
